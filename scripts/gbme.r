...
gbme <- function(#data
  ...
  #details of output
  NS = 100000,
  #number of scans of mcmc to run
  odens = 100,
  #output density
  owrite = T,
  ...
  bfilename = "B")
{
  ...
  if (directed == T) {
    cnames <-
      c(
        ...
        "scan",
        "ll",
        paste("bd", seq(1, rd, length = rd), sep = "")[0:rd],
        "b0",
        ...
        "s2e",
        "rho",
        paste("s2z", seq(1, k, length = k), sep = "")[0:k]
      )
  }
  ...
  if (estart == T) {
    tmp <- gbme.glmstart(Y, Xd, Xs, Xr, N = N, fam, k, directed)
    priors <- tmp$priors
    startv <- tmp$startv
  }
  
  if (directed == T) {
    pi.Sab <- priors$pi.Sab
    pi.s2u <- priors$pi.s2u
    pi.s2v <- priors$pi.s2v
    pi.s2z <-
      priors$pi.s2z
    pim.bd <- priors$pim.bd
    piS.bd <- priors$piS.bd
    pim.b0sr <- priors$pim.b0sr
    piS.b0sr <- priors$piS.b0sr
    beta.d <- startv$beta.d
    beta.u <- startv$beta.u
    s <- startv$s
    r <- startv$r
    z <- startv$z
  }
  ...
  tmp <-
    TuTv(n)   #unit level effects design matrix for u=yij+yji, v=yij-yji
  Tu <- tmp$Tu
  Tv <- tmp$Tv
  ...
  if (directed == T) {
    Sab0 <-
      matrix(c(pi.Sab[1], pi.Sab[2], pi.Sab[2], pi.Sab[3]),
             nrow = 2,
             ncol = 2)
    v0 <- pi.Sab[4]
  }
  ...
  for (ns in 1:NS) {
    ...
    if (fam != "gaussian") {
      theta <-
        theta.betaX.d.srE.z(beta.d, Xd, s, s * (1 - directed) + r * directed, E, z)
    }
    ...
    if (fam == "gaussian" & any(is.na(Y))) {
      ...
      mu <-
        theta.betaX.d.srE.z(beta.d, Xd, s, s * (1 - directed) + r * directed, E *
                              0, z)
      ...
      for (i in imiss) {
        for (j in (1:n)[is.na(Y[i, ])]) {
          if (is.na(Y[i, j]) & is.na(Y[j, i]))       {
            smp <-
              rmvnorm(c(mu[i, j], mu[j, i]), matrix(
                se * c(1, rho, rho, 1),
                nrow = 2,
                ncol = 2
              ))
            ...
            theta[i, j] <-
              rmvnorm(mu[i, j] + rho * (theta[j, i] - mu[j, i]), sqrt(se * (1 - rho ^
                                                                              2)))
          }
        }
      }
    ...
    if (directed == T) {
      su <-
        rse.beta.d.gibbs(pi.s2u[1], pi.s2u[2], u, XTu, s, r, beta.d)  #for rho, se
    ...
    if (directed == F) {
      su <-
        rse.beta.d.gibbs(pi.s2u[1], pi.s2u[2], u, XTu, s, NULL, beta.d) #for se
      se <- su / 4
    }
    ...
    if (directed == F) {
      s2a <- 1 / rgamma(1, pi.s2a[1] + n / 2 , pi.s2a[2] + sum(a ^ 2) / 2)
    }
    ...
    if (k > 0) {
      ...
      sz <- 1 / rgamma(k, pi.s2z[, 1] + n / 2, pi.s2z[, 2] + diag(t(z) %*% z) /
                         2)
      
      #Gibbs for zs, using regression
      res <-
        theta - theta.betaX.d.srE.z(beta.d, Xd, s, s * (1 - directed) + r * directed, 0 *
                                      E, 0 * z) #theta-(beta*x+a+b)
      s.ares <- se * (1 + rho) / 2
      for (i in sample(1:n))      {
        ...
        muzi <-
          Szi %*% (t(Zi) %*% ares / s.ares)                      #cond mean of z[i,]
    ...
    if (fam != "gaussian") {
      E <-
        theta - theta.betaX.d.srE.z(beta.d, Xd, s, s * (1 - directed) + r * directed, E *
                                      0, z)
      ...
      if (fam == "poisson") {
        ...
        mu <- exp(theta)
        M.lhr <-
          dpois(Y, mup, log = T) - dpois(Y, mu, log = T)
      }
      
      if (fam == "binomial") {
        mup <- exp(theta.p) / (1 + exp(theta.p))
    ...
    if (ns %% odens == 0) {
      ...
      if (fam == "poisson") {
        lpy.th <- sum(dpois(Y, exp(theta), log = T), na.rm = T)
      }
      if (fam == "binomial") {
        lpy.th <- sum(dbinom(Y, N, exp(theta) / (1 + exp(theta)),
                             log = T), na.rm = T)
      }
      ...
      if (fam == "gaussian") {
        ...
        v <- tmp$v                  #v=yij-yji,  i<j
        if (directed == T) {
          lpy.th <- sum(dnorm(u, 0, sqrt(su), log = T) + dnorm(v, 0, sqrt(sv), log =
                                                                 T))
        }
        if (directed == F) {
          lpy.th <- 2 * sum(dnorm(u, 0, sqrt(su), log = T))
        }
      }
      if (directed == F) {
        lpy.th <- lpy.th / 2
        out <-
          round(c(k, ns, lpy.th, beta.d, beta.u, s2a, se, sz[0:k]), oround)
      }
      if (directed == T) {
        out <- round(c(
          k,
...
TuTv <- function(n) {
  ...
  for (i in 1:(n - 1)) {
    ...
    if (i > 1) {
      for (j in 1:(i - 1)) {
        tmp <- cbind(tmp, rep(0, n - i))
      }
    }
    tmp <- cbind(tmp, rep(1, n - i))
    tmpu <- cbind(tmp, diag(1, n - i))
    tmpv <- cbind(tmp, -diag(1, n - i))
    Xu <- rbind(Xu, tmpu)
    Xv <- rbind(Xv, tmpv)
  }
...
rbeta.d.sr.gibbs <- function(u, v, su, sv, piS.bd, mu, Sab) {
  ...
  iSab <-
    rbind(cbind(diag(rep(1, n)) * Sab[2, 2] / del , -diag(rep(1, n)) * Sab[1, 2] /
                  del),
          cbind(-diag(rep(1, n)) * Sab[1, 2] / del, diag(rep(1, n)) * Sab[1, 1] /
                  del))
  ...
  list(beta.d = beta.sr[(rd > 0):rd], s = beta.sr[rd + 1:n], r = beta.sr[rd +
                                                                           n + 1:n])
}
####
...
rse.beta.d.gibbs <- function(g0, g1, x, XTx, s, r, beta.d) {
  n <- length(s)
  1 / rgamma(1, g0 + choose(n, 2) / 2, g1 + .5 * sum((x - XTx %*% c(beta.d, s, r)) ^
                                                       2))
}
####
...
rwish <- function(S0, nu) {
  S <- S0 * 0
  for (i in 1:nu) {
    z <- rmvnorm(rep(0, dim(as.matrix(S0))[1]), S0)
    S <- S + z %*% t(z)
  }
  S
}
...
proc.rr <- function(Y, X) {
  ...
  Ahalf <-
    eA$vec[, 1:k] %*% diag(sqrt(eA$val[1:k]), nrow = k) %*% t(eA$vec[, 1:k])
...
gbme.glmstart <- function(Y, Xd, Xs, Xr, N = NULL, fam, k, directed) {
  ...
  if (rd > 0) {
    fit1 <-
      glm(y ~ -1 + as.factor(x[, 1]) + as.factor(x[, 2]) + x[, -(1:2)], family =
            fam)
  }
  if (rd == 0) {
    fit1 <- glm(y ~ -1 + as.factor(x[, 1]) + as.factor(x[, 2]), family = fam)
  }
  ...
  la <- lm(s ~ -1 + cbind(rep(.5, n), Xs))
  a <- la$res
  lb <- lm(r ~ -1 + cbind(rep(.5, n), Xr))
  b <- lb$res
  ...
  la <- lm(s1 ~ -1 + cbind(rep(.5, n), Xs))
  a <- la$res
  lb <- lm(r1 ~ -1 + cbind(rep(.5, n), Xr))
  b <- lb$res
  ...
  for (m in 1:dim(x)[1]) {
    if (!is.na(y[m])) {
      Res[x[m, 1], x[m, 2]] <- fit1$res[l]
      l <- l + 1
    }
  }
  ######
  Res[Res > quantile(Res, .95, na.rm = T)] <-
    quantile(Res, .95, na.rm = T)
  diag(Res) <- rep(0, n)
  ...
  if (rd > 0) {
    pim.bd <- c(beta.d)
    piS.bd <-
      as.matrix((n * (n - 1)) ^ 2 * summary(fit1)$cov.unscaled[(2 * n):length(fit1$coef),
  ...
  if (rd == 0) {
    beta.d <- pim.bd <- NULL
    piS.bd <- matrix(nrow = 0, ncol = 0)
  }
  
  if (directed == T) {
    pim.b0sr <- beta.u
    piS.b0sr <-
      rbind(cbind(summary(la)$cov[-1, -1], matrix(0, nrow = rs, ncol = rr)),
            # cbind(matrix(0,nrow=rs,ncol=rr),summary(lb)$cov[-1,-1]))
            # error found 7-7-6
            cbind(matrix(0, nrow = rr, ncol = rs), summary(lb)$cov[-1, -1]))
    
    piS.b0sr <-
      rbind(c(summary(la)$cov[1, -1], summary(lb)$cov[1, -1]), piS.b0sr)
  ...
  if (directed == F) {
    ...
    beta.u <- tmp2$coef
    pi.s2a <- c(2, var(tmp2$res))
    
    pim.b0s <-
      tmp2$coef
    piS.b0s <- summary(tmp2)$cov * n * n
  }
